...
## 📊 자동 보고서 출력 (PDF & 편집 가능한 PPT) 사용 안내

기존에 적재된 데이터를 바탕으로 경영진(장사장님 등) 보고용 **"생산성 향상 보고서"**와 **"가스원단위 절감 보고서"**를 자동 생성하여 즉시 다운로드할 수 있습니다.

### 1. 주요 특징
- **추가 입력 Zero**: 이미 DB에 저장된 `production_records`, `gas_records`, `benchmarks` 데이터만을 100% 활용합니다.
- **편집 가능한 PPT (.pptx)**: `pptxgenjs` 네이티브 차트 엔진을 탑재하여, 파워포인트로 다운로드받은 파일 안에서 **표와 차트의 숫자/라벨을 직접 클릭해 수정**할 수 있습니다!
- **최적화된 인쇄용 PDF**: 인쇄 전용 CSS(`@media print`)가 적용되어 브라우저 인쇄(`window.print()`)나 PDF 변환 시 메뉴바가 자동으로 제거되고 A4 규격에 깔끔하게 맞춤 출력됩니다.
- **자동 진단 코멘트 생성**: 목표 미달 라인, 낭비 호기, 벤치마크 대비 개선 여지 문장이 실시간 연산되어 삽입됩니다.

### 2. 사용 방법
1. 좌측 메뉴에서 **[보고서 출력(PDF/PPT)]**(`/reports`)을 클릭합니다.
2. 상단 바에서 **보고서 종류**(생산성 / 가스원단위)와 **대상 연도**(2026 등)를 선택합니다.
3. 화면에서 실시간 렌더링 결과를 확인한 뒤 원하는 버튼을 클릭합니다:
   - **`[PDF 인쇄 모드]`**: 브라우저 인쇄 창을 열어 깔끔한 A4 레이아웃으로 바로 출력/저장합니다.
   - **`[PDF 바로 저장]`**: 화면 차트를 고화질 캔버스로 변환하여 `.pdf` 파일로 즉시 다운로드합니다.
   - **`[PPT로 저장 (.pptx)]`**: 태웅 브랜드 색상(블루/네이비)이 적용된 16:9 슬라이드 파워포인트 파일을 생성하여 다운로드합니다.
