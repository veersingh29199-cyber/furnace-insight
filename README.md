...
입력 화면은 `/input` 아래에 있습니다.

### 생산 실적
- 경로: `/input/production`
- 방식 1: 표 형태로 직접 입력
- 방식 2: 엑셀 붙여넣기 또는 파일 업로드
- 방식 3: 단건 폼 입력
- 같은 `(월, 라인, 제품, 교대)` 조합은 `upsert`로 덮어씁니다.

### 월 가스검침
- 경로: `/input/gas-monthly`
- 호기별 검침량과 장입량을 함께 입력합니다.
- 같은 `(월, 호기)` 조합은 새로 만들지 않고 수정합니다.
- 파일 업로드나 붙여넣기 시 합계열, 0, 빈칸은 자동 제외합니다.

### 일일 가스검침
- 경로: `/input/gas-daily`
- 일자 x 호기 그리드에 바로 타이핑할 수 있습니다.
- Excel/CSV를 업로드하면 미리보기 후 저장합니다.
- 같은 `(일자, 호기, 교대)` 조합은 `upsert`로 처리합니다.

### 공통 기능
- 자동 저장과 임시 저장을 지원합니다.
- Ctrl+S로 저장할 수 있습니다.
- 저장 전 미리보기에서 계산 결과를 확인할 수 있습니다.
- 숫자 입력은 음수와 문자를 막고, 천 단위 구분을 표시합니다.
## 스마트 파일 임포트
...
4. 열 매핑 확인
5. 미리보기 검증
6. 정상 행만 Supabase `upsert`

### 지원 데이터셋
- 일일 가스 검침: `gas_daily_readings`
- 월 가스: `gas_records`
- 생산 실적: `production_records`
- 생산 상세 원장(R9/P5/P8/P15 계열): `/import`에서 자동 감지 후 `production_records`로 월별 집계
- 전사 월별: `gas_company_monthly`

### 재사용 기능
- 별칭 관리: `/settings`의 `임포트 별칭` 탭
- 템플릿 저장: 같은 양식은 다음부터 자동 적용
- upsert 키: 같은 고유키가 있으면 새로 추가하지 않고 수정

### 운영 팁
- 열 이름이 제각각이어도 자동 감지와 수동 매핑을 함께 사용할 수 있습니다.
- 오류 행은 미리보기에서 확인하고, 정상 행만 저장됩니다.
- 업로드 후 첫 화면에서 시트가 자동으로 선택되며, 템플릿이 있으면 자동으로 불러옵니다.
